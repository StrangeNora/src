...
public class UpdatePatientPanel extends JPanel {
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            JFrame frame = new JFrame("Update Patient Panel");
            frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
            frame.setSize(400, 200);  // Set the desired size (width x height)
            ...
            // Set the desired location of the window
            int windowWidth = frame.getWidth();
            int windowHeight = frame.getHeight();
            int x = (screenWidth - windowWidth) / 2;  // Center horizontally
            int y = (screenHeight - windowHeight) / 2; // Center vertically
            
            frame.setLocation(x, y);
            frame.setContentPane(new UpdatePatientPanel());
